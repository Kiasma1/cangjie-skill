...
4. ★**(实战复盘) 阶段 0 写完后没立刻问用户产出路径**,导致用户实际要手册,但流程已进入 skill 模式(模块/12/15).修改版见 SKILL.md "产出路径分支决策".
5. ★**(实战复盘) 对 PDF 读完正文第一页才发现是扫描本.**应在阶段 0 之前走完 `SKILL.md > 阶段 -1` 的体检.

## 相关参考

- 上一阶段执行后立刻进入本阶段,不要在阶段 -1 未做的情况下开始 OCR 全文.
- 阶段 0 完成后: **立刻问用户 "skill / 手册 / 两份都要"**,再进入阶段 1.
